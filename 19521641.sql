...
(
	...
	PRIMARY KEY(MAKH), -- TẠO KHÓA CHÍNH NGAY TRONG BẢNG
	HOTEN VARCHAR(40),
	DCHI VARCHAR(50),
	SODT VARCHAR(20),
...
--Nhap lieu bang khách hàng
INSERT INTO KHACHHANG VALUES ('KH01','Nguyen Van A','731, Tran Hung Dao, Q 5, Tp HCM','08823451','22/10/1960',13000000,'22/10/2006')
...
--2.THÊM THUỘC TÍNH GHICHU VAO BANG SANPHAM
ALTER TABLE dbo.SANPHAM
ADD GHICHU VARCHAR(20) 

--3.THÊM THUỘC TÍNH LOAIKH VAO BANG KHACHHANG
...
ALTER TABLE dbo.KHACHHANG
ADD CONSTRAINT CHK_DK CHECK (NGDK > NGSINH)
...
SELECT KHACHHANG.MAKH, HOTEN FROM KHACHHANG INNER JOIN HOADON ON HOADON.MAKH = KHACHHANG.MAKH WHERE NGHD = '1/1/2007'

--9.	In ra số hóa đơn, trị giá các hóa đơn do nhân viên có tên “Nguyen Van B” lập trong ngày 28/10/2006.
SELECT SOHD, TRIGIA FROM NHANVIEN INNER JOIN HOADON ON HOADON.MANV = NHANVIEN.MANV WHERE ( HOTEN = 'Nguyen Van B' AND NGHD = '28/10/2006')

--10.	In ra danh sách các sản phẩm (MASP,TENSP) được khách hàng có tên “Nguyen Van A” mua trong tháng 10/2006.
SELECT SANPHAM.MASP, TENSP FROM KHACHHANG,HOADON,CTHD,SANPHAM 
WHERE KHACHHANG.HOTEN = 'Nguyen Van A' AND HOADON.MAKH = KHACHHANG.MAKH AND CTHD.SOHD = HOADON.SOHD AND CTHD.MASP = SANPHAM.MASP AND (MONTH(HOADON.NGHD) = 10 AND YEAR(HOADON.NGHD) = 2006) 

--11.	Tìm các số hóa đơn đã mua sản phẩm có mã số “BB01” hoặc “BB02”.
SELECT DISTINCT SOHD FROM CTHD WHERE CTHD.MASP IN ('BB01', 'BB02') 

--12.	Tìm các số hóa đơn đã mua sản phẩm có mã số “BB01” hoặc “BB02”, mỗi sản phẩm mua với số lượng từ 10 đến 20.
SELECT  DISTINCT SOHD FROM CTHD WHERE CTHD.MASP IN ('BB01', 'BB02') AND (SL BETWEEN 10 AND 20)

--13.	Tìm các số hóa đơn mua cùng lúc 2 sản phẩm có mã số “BB01” và “BB02”, mỗi sản phẩm mua với số lượng từ 10 đến 20.
-- SQL1 INTERSECT SQL2 : PHÉP GIAO
SELECT DISTINCT CT.SOHD FROM CTHD CT WHERE CT.MASP ='BB01' AND (CT.SL BETWEEN 10 AND 20)
INTERSECT SELECT DISTINCT CT.SOHD FROM CTHD CT WHERE CT.MASP = 'BB02' AND (CT.SL BETWEEN 10 AND 20)	

--14.	In ra danh sách các sản phẩm (MASP,TENSP) do “Trung Quoc” sản xuất hoặc các sản phẩm được bán ra trong ngày 1/1/2007.
-- SQL1 UNION SQL2 : PHÉP HOẶC
SELECT MASP, TENSP FROM SANPHAM WHERE NUOCSX = 'Trung Quoc' UNION
SELECT SANPHAM.MASP, TENSP FROM SANPHAM,CTHD,HOADON WHERE NGHD = '1/1/2007' AND CTHD.MASP = SANPHAM.MASP AND CTHD.SOHD = HOADON.SOHD

--15.	In ra danh sách các sản phẩm (MASP,TENSP) không bán được. 
--C1
SELECT MASP, TENSP FROM SANPHAM WHERE MASP NOT IN (SELECT CTHD.MASP FROM CTHD )
--C2
SELECT MASP, TENSP FROM SANPHAM WHERE NOT EXISTS (SELECT CTHD.MASP FROM CTHD WHERE CTHD.MASP = SANPHAM.MASP )

--16.	In ra danh sách các sản phẩm (MASP,TENSP) không bán được trong năm 2006.
SELECT MASP, TENSP FROM SANPHAM WHERE MASP NOT IN (SELECT CTHD.MASP FROM CTHD,HOADON WHERE YEAR(NGHD) = 2006 AND CTHD.SOHD = HOADON.SOHD)

--17.	In ra danh sách các sản phẩm (MASP,TENSP) do “Trung Quoc” sản xuất không bán được trong năm 2006. 
SELECT MASP, TENSP FROM SANPHAM WHERE NUOCSX = 'Trung Quoc' EXCEPT 
SELECT SANPHAM.MASP,SANPHAM.TENSP FROM CTHD,HOADON,SANPHAM WHERE YEAR(NGHD) = 2006 AND NUOCSX = 'Trung Quoc' AND CTHD.MASP = SANPHAM.MASP AND CTHD.SOHD = HOADON.SOHD

--18.	Tìm số hóa đơn đã mua tất cả các sản phẩm do Singapore sản xuất.
SELECT SOHD FROM HOADON WHERE  NOT EXISTS -- những hóa đơn mà 
(SELECT MASP FROM SANPHAM WHERE NUOCSX ='Singapore' AND  NOT EXISTS -- không có mã sản phẩm Singaore nào không có
(SELECT CTHD.MASP FROM CTHD WHERE CTHD.SOHD = HOADON.SOHD AND CTHD.MASP = SANPHAM.MASP)) --trong các sản phẩm đã được mua

--19.	Tìm số hóa đơn trong năm 2006 đã mua ít nhất tất cả các sản phẩm do Singapore sản xuất.
SELECT SOHD FROM HOADON WHERE YEAR(NGHD) = 2006 AND NOT EXISTS
(SELECT * FROM SANPHAM WHERE NUOCSX ='Singapore' AND MASP NOT IN
(SELECT MASP FROM CTHD WHERE CTHD.SOHD = HOADON.SOHD))

--20.	Có bao nhiêu hóa đơn không phải của khách hàng đăng ký thành viên mua?
SELECT COUNT(SOHD) FROM HOADON WHERE MAKH IS NULL

--21.	Có bao nhiêu sản phẩm khác nhau được bán ra trong năm 2006.
SELECT COUNT(DISTINCT MASP) FROM CTHD,HOADON  WHERE YEAR(NGHD) = 2006 AND CTHD.SOHD = HOADON.SOHD
	
--22.	Cho biết trị giá hóa đơn cao nhất, thấp nhất là bao nhiêu ?
SELECT MIN(TRIGIA) AS MINLUONG , MAX(TRIGIA) AS MAXLUONG FROM HOADON  
...
SELECT SOHD FROM HOADON WHERE TRIGIA IN (SELECT MAX(TRIGIA) FROM HOADON WHERE YEAR(NGHD) = 2006)